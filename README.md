# 🎯 Awesome Study Resources

> 精选的高质量学习资源收藏夹 —— 记录那些把知识讲得清清楚楚的视频、博客、论文、代码和公众号。

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

---

## 📚 目录

### 🤖 人工智能 & 机器学习
| 分类 | 描述 |
|------|------|
| [深度学习](./categories/deep-learning/README.md) | 神经网络、CNN、RNN、Transformer 等 |
| [机器学习](./categories/machine-learning/README.md) | 传统 ML 算法、特征工程、模型评估等 |
| [NLP 自然语言处理](./categories/nlp/README.md) | 文本处理、语言模型、对话系统等 |
| [计算机视觉](./categories/computer-vision/README.md) | 图像处理、目标检测、图像生成等 |


### 💻 编程 & 软件开发
| 分类 | 描述 |
|------|------|
| [编程语言](./categories/programming-languages/README.md) | Python、Java、Go、Rust 等 |
| [前端开发](./categories/frontend/README.md) | HTML、CSS、JavaScript、React、Vue 等 |
| [后端开发](./categories/backend/README.md) | Web 框架、API 设计、服务端架构等 |
| [系统设计](./categories/system-design/README.md) | 架构设计、分布式系统、微服务等 |
| [数据库](./categories/database/README.md) | SQL、NoSQL、数据建模等 |
| [DevOps](./categories/devops/README.md) | CI/CD、容器化、云服务等 |

### 📐 基础学科
| 分类 | 描述 |
|------|------|
| [数学](./categories/mathematics/README.md) | 线性代数、概率统计、微积分等 |
| [算法与数据结构](./categories/algorithms/README.md) | 经典算法、LeetCode 题解等 |

### 🌟 其他
| 分类 | 描述 |
|------|------|
| [优质创作者](./categories/creators/README.md) | 值得关注的 UP 主、博主、公众号等 |
| [学习方法](./categories/learning-methods/README.md) | 高效学习、笔记方法等 |

---

## 🏷️ 资源类型图例

| 标签 | 含义 |
|------|------|
| 📺 | 视频 |
| 📝 | 博客/文章 |
| 📄 | 论文 |
| 💻 | 代码/项目 |
| 📖 | 书籍 |
| 🎧 | 播客/音频 |
| 📱 | 公众号/Newsletter |
| 🔧 | 工具 |

---

## ⭐ 精选推荐

> 这里放置最值得推荐的资源

*待添加...*

---

## 🤝 贡献

欢迎提交 PR 或 Issue 来分享你发现的优质资源！

请查看 [贡献指南](./CONTRIBUTING.md) 了解如何贡献。

---

## 📜 License

[![CC0](https://licensebuttons.net/p/zero/1.0/88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)

本项目采用 [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) 协议，你可以自由使用。
